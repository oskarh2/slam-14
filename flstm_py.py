...
def lstm_exec(dataX, dataY,fechas, df3,num_epochs,learning_rate ):
    input_size = 1
    hidden_size = 2
    num_layers = 1
    num_classes = 1
...
# Train the model
    ...
    #plt.show()
    return expected, predictions, predictions1
    
#df = pd.read_csv('loterias_test.csv')
df = pd.read_csv('boyaca_test.csv')
...
df2 = df1.tail(30)
df_val = pd.DataFrame(columns=['nro', 'vnro', 'params','mae','mse','rmse'])
for column in df2.columns[5:]:
    ...
    dataX1 = dataX
    dataY1 = dataY
    lnum_epochs = [3000,4000,5000 ]
    learning_rate =[0.1,0.01,0.001]
    for i in lnum_epochs:
        for j in learning_rate:
            print('colum:', column, 'i:',i,' j:',j)
            expected ,predictions, predictions1 = lstm_exec(dataX1, dataY1,fechas,df3,int(i),float(j))
            ...
            params = 'i:'+ str(i)+' j:'+str(j)
            df_val = df_val.append({'nro' : column_names[1]  , 'vnro' : predictions1,'params': params ,'mae': mae ,'mse':mse ,'rmse': rmse},    ignore_index=True )
print(df_val)